...
const IDL = require("../target/idl/vesting_program.json");
...
export async function envSetup() {
  ...
  const program = new Program<VestingProgram>(IDL, provider);

  const employerProvider = new BankrunProvider(context);
  employerProvider.wallet = new NodeWallet(employer);
  const employerProvidedProgram = new Program<VestingProgram>(
    IDL,
    employerProvider
  );
  ...
  const beneficiaryProvidedProgram = new Program<VestingProgram>(
    IDL,
    beneficiaryProvider
  );
